...
class GLM(LLM):
    ...
    def post_attention_compute(
        ...
        # 将 hidden_states 分成两半
        gate, up = hidden_states.chunk(2, dim=-1)
        
        # 应用 SiLU (Swish) 激活函数并执行乘法
        # 这替代了 vllm._custom_ops.silu_and_mul
        gate = F.silu(gate)
        ...
        hidden_states = residual + hidden_states
        ffn_end =  time.time()
        # print(f"ffn time :{ffn_end - ffn_start}")
    ...
    def apply_rotary_pos_emb(self, q: torch.Tensor, k: torch.Tensor, position_ids: torch.Tensor) -> torch.Tensor:
        ...
        # q,k = custom_apply_rotary_emb(q, k, position_ids, self.cos_sin_cache)
        # bsz = q.shape[0]
        ...
        # 确保输入形状正确
        bsz, seq_len, hidden_dim = q.shape[0], q.shape[1], q.shape[2]
